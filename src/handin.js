...
window.addEventListener("load", () => {
  ...
  initDom();//モーダル要素とモーダルボタン要素を取得してる。
  initThree();//three.jsの設定してる。
  //initAudio()を実行、成功したらdecodeAudioDataの第２引数にbufferが入る。
  //thenを実行する。モーダルをリムーブ
  initAudio().then(data => {
    //classList:要素のクラスリストを削除する。
    modalButton.classList.remove("is-Hidden");//modalボタン要素を。(隠す要素をはずすから)
...
const initThree = () => {
  ...
  geo.setAttribute('uv', uvs);
        ...
        for(var i=0;i<num;i++){
          ...
          let noise_value = Perlin.noise.perlin3(x,y,z);
